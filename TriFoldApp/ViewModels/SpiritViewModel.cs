...
public class SpiritViewModel : BaseViewModel
{
	...
	public SpiritViewModel(ITrackerRepository repository)
	{
	...
	private async Task LoadAsync()
	{
		...
		var templates = (await _repository.GetTaskTemplatesAsync(TaskCategory.Spirit)).ToList();
		var completedMinutesByTemplateId = tasks
			.Where(t => t.Category == TaskCategory.Spirit && t.IsCompleted && t.DueDate.Date <= DateTime.Today && !string.IsNullOrWhiteSpace(t.TaskTemplateId))
			.GroupBy(t => t.TaskTemplateId!)
			.ToDictionary(g => g.Key, g => g.Sum(t => t.DurationMinutes ?? 0));

		foreach (var task in tasks.Where(t => t.Category == TaskCategory.Spirit).OrderBy(t => t.DueDate))
		{
			Tasks.Add(task);
		}
	...
	private async Task AddStandardTaskAsync()
	{
		...
		await _repository.AddTaskTemplateAsync(NewStandardTaskTitle, TaskCategory.Spirit);
